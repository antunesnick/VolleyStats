class Player{
  ...
  insertPlayer(player, db){
    try {
      ...
      const info = sql.run(player.cpf, player.nome, player.dataNasc, player.numCamisa, player.rg, player.altura, player.posicaoId, player.foto);
  ...
  deletePlayer(id, db){
    try {   const sql = db.prepare('DELETE FROM Jogadores WHERE id = ?');
            sql.run(id);
    } catch (e) {
        throw e;
    }
    ...
    updatePlayer(player, db){
        try {
            const sql = db.prepare('UPDATE Jogadores SET cpf = ?, nome = ?, dataNasc = ?, numCamisa = ?, rg = ?, altura = ?, posicao_id = ?, foto = ? WHERE id = ?');
            sql.run(player.cpf, player.nome, player.dataNasc, player.numCamisa, player.rg, player.altura, player.posicaoId, player.foto, player.id);
        } catch (e) {
            throw e;
        }
    }
